	...
	public int [] movimientoJugador(boolean jugador) {
		...
		while(!posicionCorrecta) {
			for(int i=0;i<2;i++) {
				...
				char charPos = pos.charAt(0);
				//comprobar que es un número
				if(charPos!=0 || charPos!=1 || charPos!=2) { 
					if(Character.isDigit(charPos)) {  //si es un número lo convertimos en integer
						posicion[i] = Integer.parseInt(pos); 
						coordIncor = true;
					}else { // si no ponemos un número fuera del rango 0-2 para error
						posicion[i] = 7;
						coordIncor = false;
					}
				}
			...
			if(posicion[0]<0 || posicion[0]>2 || posicion[1]<0 || posicion[1]>2){
				...
				System.out.println("Error en coordenadas. Escribe 0, 1 o 2.");	
			}
		}
		return posicion;
	}//movimientoJugador
	
	public boolean comprobarTablero(int [] posicion) {//evitar colocar en casilla ocupada
		boolean comprobar;
		if(tablero[posicion[0]][posicion[1]]=='X' || tablero[posicion[0]][posicion[1]]=='O') {
			comprobar = false;
		}else {
			comprobar = true;
		}
